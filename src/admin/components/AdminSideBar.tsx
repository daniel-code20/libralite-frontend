      ...
      <div className="h-full flex flex-col justify-between px-6 py-4">
        <div>
          ...
          <nav className="flex flex-col space-y-4 mt-10">
            <Link to={'/admin-principal'} className={`flex items-center p-2 rounded-md ${isActive('/admin-principal') ? 'bg-blue-100' : ''}`}>
              <FaHome className={`mr-3 ${isActive('/admin-principal') ? 'text-blue-500' : 'text-gray-500'}`} />
              <h1 className={`text-black ${isActive('/admin-principal') ? 'font-semibold text-blue-500' : ''}`}>
                Principal
              </h1>
            </Link>
            <Link to={'/admin-buyslist'} className={`flex items-center p-2 rounded-md ${isActive('/admin-buyslist') ? 'bg-blue-100' : ''}`}>
              <FaShoppingCart className={`mr-3 ${isActive('/admin-buyslist') ? 'text-blue-500' : 'text-gray-500'}`} />
              <h1 className={`text-black ${isActive('/admin-buyslist') ? 'font-semibold text-blue-500' : ''}`}>
                Compras
              </h1>
            </Link>
            <Link to={'/admin-reservationlist'} className={`flex items-center p-2 rounded-md ${isActive('/admin-reservationlist') ? 'bg-blue-100' : ''}`}>
